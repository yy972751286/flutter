...
class _MyHomePageState extends State<MyHomePage> {
  String _counter = "等待传值";

  //交互的通道名称，flutter和native是通过这个标识符进行相互间的通信
  static const communicateChannel = MethodChannel('hft_soft');

  //异步执行调用原生方法，保持页面不卡住，因为调用原生的方法可能没实现会抛出异常，所以trycatch包住
  Future<void> _incrementCounter(flutterPara) async {
    try {
      //原生方法名为callNativeMethond,flutterPara为flutter调用原生方法传入的参数，await等待方法执行
      final result = await communicateChannel.invokeMethod('callNativeMethond:', {"asda": flutterPara,"122":flutterPara});
